...
from utils.trusted_device import verify_websocket_trusted_device
...
async def ui_bridge_ws(websocket:WebSocket):
	'首页建立的参数桥接通道。';A=websocket
	if not await verify_websocket_trusted_device(A):return
	B=(A.query_params.get('client_id')or'').strip()
